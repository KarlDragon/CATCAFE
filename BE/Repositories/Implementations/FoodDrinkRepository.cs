...
public class FoodDrinkRepository : IFoodDrinkRepository
{
    ...
    {
        return await _context.FoodDrinks.Where(fd => fd.IsActive)
                                    .AsNoTracking()
    ...
    {
        return await _context.FoodDrinks.AnyAsync(fd => fd.Name == foodDrinkName && fd.IsActive);
    }

    public async Task<Dictionary<int, decimal>> GetFoodDrinkPriceByIdsAsync( List<int> foodDrinkIds )
    {
        var foodDrinks = await _context.FoodDrinks.Where(fd => foodDrinkIds.Contains(fd.FoodDrinkID) && fd.IsActive).ToListAsync();
